...
'''
StandardScaler: maps the values into the  standrad distribution (mean=0 std=1)
we will apply it on all numeric values. 
You can see that even the StandardScaler uses the fit and transform methods.
fit learns the values in order to normalize and transform applies it. 
In order to learn it and then apply it directly on the train set, you can 
...
'''
Our goal is to build a supervised machine learning model to predict whether 
a given user is recommending a product or not. 
For simplicity reasons we will consider only columns with numeric data. 
Let's create a copy dataframe only with numeric data:
'''
df_copy=df[["Clothing ID","Age","Rating","Recommended IND","Positive Feedback Count"]].copy()
df_copy.head()
print(df_copy.head())
'''
you could have used also the function _get_numeric_data() instead of
 explicitly selecting 
 
 Data Preperation
----------------------------
Let's start by creating: 
1. matrix X that will contain the only the features,
2. vector y with all the labels.  The Recommended IND column
   the customer recommends buying  '1' means recommend and '0' otherwise.
...
# Use the classifier model   and apply it  on new data ($X_test) in order
