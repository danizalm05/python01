...
class App(QWidget):
    def __init__(self):
        ...
        blank = np.zeros((cv_img.shape[:2]), dtype='uint8')

        circle = cv2.circle(blank.copy(), (cv_img.shape[1]//4  ,cv_img.shape[0]//2), 130, 355, -1)
        rectangle = cv2.rectangle(blank.copy(), (60,60), (470,470), 255, -1)
        cv2.imshow('circle', circle)
        cv2.imshow('Blank Image', rectangle)
        weird_shape = cv2.bitwise_and(circle, rectangle)
        cv2.imshow('weird_shape', weird_shape)
        masked = cv2.bitwise_and(cv_img,cv_img,mask=weird_shape)
        cv2.imshow('Weird Shaped Masked Image', masked)


        # convert the image to Qt format
        qt_img = self.convert_cv_qt(masked)
        # display it
