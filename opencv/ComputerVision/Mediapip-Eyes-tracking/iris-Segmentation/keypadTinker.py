'''
https://www.youtube.com/watch?v=7El6Yu2rYkw
        ...
        if (x + w > col[0] and x + w < col[1]) and (y + h > row[i] and y + h < row[i] + 70):
            ...
            if (x + w > col[0] and x + w < col[1]) and (y + h > row[i] and y + h < row[i] + 70):
                ...
                if counter >= 30:
                    ...
                    cv2.putText(rgb, str(index_col_one[i]), (145, row[i] + 40), cv2.FONT_HERSHEY_TRIPLEX, 0.70,
                                (200, 0, 0))
                    counter = 0
            else:
                counter = 0
    ...
    for i in range(0, 4):
        ...
        if (x + w > col[1] and x + w < col[2]) and (y + h > row[i] and y + h < row[i] + 70):
            ...
            if (x + w > col[1] and x + w < col[2]) and (y + h > row[i] and y + h < row[i] + 70):
                ...
                if counter >= 30:
                    show_ind['text'] += str(index_col_two[i])
                    cv2.putText(rgb, str(index_col_two[i]), (245, row[i] + 40), cv2.FONT_HERSHEY_TRIPLEX, 0.70,
                                (200, 0, 0))
                    counter = 0
            else:
                counter = 0
    ...
    for i in range(0, 4):
        ...
        if (x + w > col[2] and x + w < col[3]) and (y + h > row[i] and y + h < row[i] + 70):
            ...
            if (x + w > col[2] and x + w < col[3]) and (y + h > row[i] and y + h < row[i] + 70):
                ...
                if counter >= 30:
                    show_ind['text'] += str(index_col_three[i])
                    cv2.putText(rgb, str(index_col_three[i]), (345, row[i] + 40), cv2.FONT_HERSHEY_TRIPLEX, 0.70,
                                (200, 0, 0))
                    counter = 0
            else:
                counter = 0
...
def display():
    ...
    if cmb2.get() == "Scales":
        ...
        button['command'] = lambda: print(l_h.get(), ',', l_s.get(), ',', l_v.get(), ',', u_h.get(), ',', u_s.get(),
                                          ',', u_v.get())
    else:
        button.place_forget()
