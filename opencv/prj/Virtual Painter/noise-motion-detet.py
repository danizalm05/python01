'''
 noise-motion-detet.py
Simple Noise-tolerant Motion Detector
...
##############
'''
last frame =  average of the last 30 frames, 
current frame =  weighted average  of the last 10 frames.

1. Resize the image for two reasons. 
        a. we don’t need the detail level further down. 
        b. make processing faster.
2. Blur the image to minimize the impact of small details.
3. Add the image to a foreground list, which will be used to calculate the first 
    image. This list can be 10 frames. We will calculate the foreground image from 
    it using a weighted average to give more weight to never frames.
4. The frame is also added to the longer background list. The output of that 
    list is a normal average.
5.  The difference is calculated of the output of the foreground list and the 
     background list.
