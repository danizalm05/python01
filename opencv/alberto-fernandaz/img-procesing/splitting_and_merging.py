...
plt.suptitle("Splitting and merging channels in OpenCV", fontsize=14,
                        fontweight='bold')
...
# Show the BGR image:
show_with_matplotlib(image_copy, "merge(b, g, r)", 1 + 6)
...
# Split the 'image_without_blue' image into its three components

(b, g, r) = cv2.split(image_without_blue)

# Show all the channels from the BGR image without the blue information:
show_with_matplotlib(cv2.cvtColor(b, cv2.COLOR_GRAY2BGR), "BGR - B (B)", 4)
show_with_matplotlib(cv2.cvtColor(g, cv2.COLOR_GRAY2BGR), "BGR - B (G)", 4 + 6)
show_with_matplotlib(cv2.cvtColor(r, cv2.COLOR_GRAY2BGR), "BGR - B (R)", 4 + 6 * 2)
...
show_with_matplotlib(cv2.cvtColor(b, cv2.COLOR_GRAY2BGR), "BGR - G (B)", 5)
show_with_matplotlib(cv2.cvtColor(g, cv2.COLOR_GRAY2BGR), "BGR - G (G)", 5 + 6)
show_with_matplotlib(cv2.cvtColor(r, cv2.COLOR_GRAY2BGR), "BGR - G (R)", 5 + 6 * 2)
...
show_with_matplotlib(cv2.cvtColor(b, cv2.COLOR_GRAY2BGR), "BGR - R (B)", 6)
show_with_matplotlib(cv2.cvtColor(g, cv2.COLOR_GRAY2BGR), "BGR - R (G)", 6 + 6)
show_with_matplotlib(cv2.cvtColor(r, cv2.COLOR_GRAY2BGR), "BGR - R (R)", 6 + 6 * 2)
